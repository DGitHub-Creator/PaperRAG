...
```
FastAPI 后端 + Vue 3 前端（CDN 单页） + Milvus 向量数据库 + PostgreSQL + Redis
```
...
┌──────────────────────────────────────────────────────────────┐
│                       前端 (Vue 3 CDN)                        │
│   index.html  │  script.js  │  style.css  │  KaTeX/Marked    │
└──────────┬───────────────────────────────────────────────────┘
...
│   │   ├── document_loader.py     # PDF 多解析器降级、结构分块、定理检测
│   │   ├── embedding.py           # 稠密向量 + BM25 稀疏向量 + 统计持久化
...
│   ├── services/
│   │   ├── agent.py             # LangChain Agent、对话管理
│   │   ├── cache.py             # Redis 缓存 + 语义缓存
│   │   ├── tools.py             # 天气查询、知识库检索工具
│   │   └── upload_jobs.py       # 文档上传任务管理
│   └── vectordb/
│       ├── milvus_client.py     # Milvus 集合管理、混合检索
