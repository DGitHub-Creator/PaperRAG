...
createApp({
    ...
    methods: {
        ...
        parseMarkdown(text) {
            ...
            // 恢复并渲染 LaTeX
            html = html.replace(/@@LATEXBLOCK(\d+)@@/g, (match, idx) => {
                ...
                if (item && typeof katex !== 'undefined') {
                    ...
                    } catch (e) { return `<pre>${item.content}</pre>`; }
                }
                return `$$${item.content}$$`;
            });
            html = html.replace(/@@LATEXINLINE(\d+)@@/g, (match, idx) => {
                const item = latexBlocks[parseInt(idx)];
                if (item && typeof katex !== 'undefined') {
                    try {
                        return katex.renderToString(item.content, { displayMode: false, throwOnError: false });
                    } catch (e) { return `$${item.content}$`; }
                }
                return `$${item.content}$`;
            });
            return html;
