...
export function useSessions() {
  ...
  async function list(): Promise<void> {
    ...
    } catch (e: unknown) { alert("加载历史记录失败：" + (e instanceof Error ? e.message : String(e))) }
  }
  ...
  async function loadMessages(sessionId: string): Promise<SessionMessagesResponse | null> {
    ...
    } catch (e: unknown) { alert("加载会话失败：" + (e instanceof Error ? e.message : String(e))); return null }
  }

  async function remove(sessionId: string): Promise<boolean> {
    if (!confirm(`确定删除会话？`)) return false
    try {
      const res = await authFetch(`/sessions/${encodeURIComponent(sessionId)}`, { method: "DELETE" })
      if (!res.ok) throw new Error("删除失败")
      sessions.value = sessions.value.filter((s) => s.session_id !== sessionId)
      return true
    } catch (e: unknown) { alert("删除失败：" + (e instanceof Error ? e.message : String(e))); return false }
  }
