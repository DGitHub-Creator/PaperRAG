...
**现状**：`backend/services/tools.py:33-43` 四个全局变量在 gunicorn 多 worker 下完全失效。
...
**现状**：无 `/health`、`/ready` 端点，Docker Compose 无应用健康检查。
...
**现状**：`logging_config.py` 提供标准化 logger，但无结构化输出，无法接入 APM。
...
**现状**：`backend/core/database.py:37` 使用 SQLAlchemy 默认连接池参数。
...
### 2.1 解除每轮 1 次检索限制 → 多轮检索

**现状**：`tools.py:238` 硬限制 `search_knowledge_base` 每轮只能调用 1 次。
...
**现状**：RAG 管线每次独立执行，不知道历史对话中提到的文档或概念。
...
### 2.3 LangGraph 添加 Checkpoint

**现状**：`rag_graph.compile()` 无 checkpointer，失败必须重头跑。
...
1. **先做 Phase 0 再谈其他** — 全局变量和健康检查是后续所有工作的基础
